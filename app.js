...
const services = [
...

  {
    ...
    price: 100,
    stockKey: "craftland_followers_fast"
  },
...
/*
  Default:
  Agar kisi service ka stock database me
  available na mile to IN STOCK rakhenge.
*/

function isServiceInStock(service) {
...
async function loadServiceStock() {
  ...
  try {
    ...
    if (error) {
      ...
      console.error(
        "SERVICE STOCK LOAD ERROR:",
        error
      );

      /*
        Stock table error hone par
        existing website ko completely
        block nahi karenge.
      */
...
async function refreshServiceStock() {
...
async function checkSelectedServiceStock(
  service
) {
  ...
  try {
    ...
    if (error) {
      ...
      /*
        Database read fail hone par
        checkout ko unnecessarily
        block nahi karenge.
      */
...
async function openCheckout(
  ...
  /*
    Fresh stock check.
    Isse agar admin ne abhi service
    OUT OF STOCK ki hai to customer
    purane page se bhi order nahi kar payega.
  */

  const inStock =
    await checkSelectedServiceStock(
      selected
    );
...
async function submitOrder() {
  ...
  */
  ...
  if (!inStock) {
    ...
    alert(
      "Ye service abhi OUT OF STOCK hai. Order submit nahi kiya ja sakta."
    );
